patches-own [
  ...
  is-on-wall?          ;; true if ants are on the wall, otherwise false
...
to setup
  ...
  setup-patches
  import-pcolors environment
  setup-maze
  create-turtles population
  [ set size 1.5
    set color red ] ;; starting point
end
...
to setup-maze
  ...
  [ ifelse pcolor = black
    [ set is-on-wall? true ]
    [ set is-on-wall? false ]
    if pcolor != white and pcolor != black
    [ set pcolor white set is-on-wall? false ]
  ]
end
...
to recolor-patch  ;; patch procedure
  if (is-on-wall? = false)
  [ set pcolor scale-color green chemical 10 0.0001 ]
end
...
to uphill-chemical  ;; turtle procedure
  let scent-ahead chemical-scent-at-angle -45
  let scent-right chemical-scent-at-angle  45
  let scent-left  chemical-scent-at-angle   0
...
to wiggle ;; turtle procedure
  rt random 40
  lt random 40
  ifelse not can-move? 1 [ rt 180 ]
   [if [is-on-wall?] of patch-ahead 1 = true [ rt 180 ]]
end
